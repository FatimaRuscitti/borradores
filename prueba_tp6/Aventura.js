class Juego {
  ...
  setup() {
    ...
    for (let i = 0; i < this.fondo.length; i++) {
      this.fondo[i] = loadImage("fondo_" + i + ".png", this.cargarImagenError);
    }
    ...
    this.textoPantallas[4] = "Envían a Fermina al extranjero para que se olvide de Florentino.";
 ...
 this.miniJuego.actualizar ();
     ...
     /*else if ((this.minumero == 12 || this.minumero == 9) && this.nuevaSecuencia) {
      if (mouseX >= this.buttonX2 && mouseX <= this.buttonX2 + this.buttonWidth && mouseY >= this.buttonY2 && mouseY <= this.buttonY2 + this.buttonHeight) {
        this.reiniciar();
      }
    } */
  ...
  imagenesytexto() {
    ...
    if (this.minumero >= 0 && this.minumero < 13) {
      ...
      } else if ((this.minumero == 9 || this.minumero == 11) && this.nuevaSecuencia) {
